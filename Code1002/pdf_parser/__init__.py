...
class PdfParser:
    ...
    def parse_pdf(self, pdf_bytes, lang='eng', ppi=150):
        '''
        ...
        ocr_df = self.get_ocr_df(pdf_bytes, surya_df, lang=lang, surya_ppi=ppi, ocr_ppi=ppi)
        ...
        merged_df = merged_df[['page_idx', 'position', 'bbox', 'label', 'text']]
    ...
    def get_ocr_df(self, pdf_bytes, surya_df, lang='eng', surya_ppi=72, ocr_ppi=150):
        ocr_images = tk.pdfbytes2imgs(pdf_bytes, ppi=ocr_ppi)
        if surya_ppi == ocr_ppi:
            buffer_size = 0
        else:
            buffer_size = 2
        ocr_df = ot.run_ocr(images=ocr_images, layout_table=surya_df, image_ppi=ocr_ppi, layout_ppi=surya_ppi, lang=lang, temp_path=temp_folder / 'sample.tiff', buffer_size=buffer_size)
        return ocr_df
